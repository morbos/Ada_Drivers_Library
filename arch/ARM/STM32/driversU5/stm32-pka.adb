...
package body STM32.PKA is
   ...
   begin
      ...
      To (Off + 1) := 0; --  Mimic
      To (Off + 2) := 0; --   ST's PARAM_END macro
   end Copy_S_To_U32;
   ...
   is
      ...
      Curve : CurveData;
   begin
      Enable_Rng;  --  Has to be running! or PKA hangs for some cases
      if Curve_Name = "secp521r1" then
         ...
         Neg := False;
      elsif Curve_Name = "sm2p256v1" then
         Curve.P :=
      ...
      elsif Mode = Field_Arithmetic then
         declare
   ...
   begin
      ...
      --   if different the signature is not verified'
      if Err /= 16#d60d# then
         Result := False;
      end if;
      return Result;
   ...
   begin
      ...
      Disable_Pka;
      return Result and (ECDSA.Result = 16#d60d#);
   end ECDSA_Valid;
...
--      Copy_S_To_U32 (Curve.P, ECDSA.Curve_Mod);
      ...
      Copy_U32_To_S (ECDSA.Y_Out, Y_Res);
...
--      Clear_Ram;
--      NegOne := (1 => 'F', others => 'F');
--      ECDSA_Math (Work => "AB*=", A => NegOne, B => NegOne, O1 => Result, O2 => Result);
      Clear_Ram;
      Curve := Common_Init (Field_Arithmetic);
      ECDSA.N_Bits := Num_Bits;
      Disable_Pka;
...
--      Zero := (1 => '0', others => '0');
--      ECDSA_Math (Work => "AB=", A => LScalar, B => Zero, O2 => Result);

      Copy_S_To_U32 (Scalar, ECDSA.Scalar);
      --  From RM:
      --  For k < 0 (i.e. a negative scalar multiplication is
      --  required) multiplier absolute value k = |-k| must be
      --  provided to the PKA. After the computation completion, the
      --  formula -P = (x, -y) can be used to compute the y coordinate
      --  of the effective final result (the x coordinate remains the
      --  same).

--      if (ECDSA.Scalar (Integer (N_By_32) - 1) and Shift_Left (1, Integer (31 - Rem_By_32))) /= 0 then -- Odd
--         for I in 0 .. N_By_32 - 1 loop
--            ECDSA.Scalar (Integer (N_By_32) - 1) := -ECDSA.Scalar (Integer (N_By_32) - 1);
--         end loop;
--         Invert := True;
--      end if;
      Copy_S_To_U32 (Curve.X, ECDSA.X);
   ...
   begin
      ...
      Montgomery.N_Bits := Num_Bits;
      Copy_S_To_U32 (Curve.P, Montgomery.Mod_In);
      StartPKA (Montgomery_Parameter_Computation_Only);
      Copy_U32_To_S (Montgomery.Param_Out, Result);
   end Montgomery_Param_Computation;
   ...
   --
   --  This func() works with 2 exceptions: the 112r[12] curves...
   --  I don't know why it fails for those curves... yet.
   ...
   begin
      Enable_Pka;
      Clear_Ram;
      --  Get Mongomery param R2 mod n
      Montgomery_Param_Computation (Param);
      Clear_Ram;
      Disable_Pka;
      Curve := Common_Init (Point_Check);
      ...
      Copy_S_To_U32 (Param, ECDSA.R2_Mod_N);
      StartPKA (Point_On_Elliptic_Curve_Fp_Check);
      Disable_Pka;
      if ECDSA.Result /= 16#d60d# then
         Xyzzy;
      end if;
      return (ECDSA.Result = 16#d60d#);
